...
export class ByteArray {
	...
	public asUint8Array(): Uint8Array {
		return new Uint8Array(this.data.buffer, 0, this.write_position);
	}
	...
	public readBytes(bytes: ByteArray, offset: number = 0, length: number = 0): void {
		...
		if (available < 0) {
			console.error("readBytes out of range");
			return;
		}
		if (length == 0) {
			length = available;
	...
	public readInt64(): number {
		if (this.validate(ByteArraySize.SIZE_OF_INT64)) {
			let low = this.data.getUint32(this._position, this.$endian == EndianConst.LITTLE_ENDIAN);
			let high = this.data.getInt32(this._position + 4, this.$endian == EndianConst.LITTLE_ENDIAN);
			this.position += ByteArraySize.SIZE_OF_INT64;
	...
	public readUInt64(): number {
		if (this.validate(ByteArraySize.SIZE_OF_UINT64)) {
			let low = this.data.getUint32(this._position, this.$endian == EndianConst.LITTLE_ENDIAN);
			let high = this.data.getUint32(this._position + 4, this.$endian == EndianConst.LITTLE_ENDIAN);
			this.position += ByteArraySize.SIZE_OF_UINT64;
			return high * 0x100000000 + low;
		}
	}
	...
	public writeInt64(value: number): void {
		if (value >= 0) {
			...
			let low = value & 0x7FFFFFFF;
			let hi = ~Math.floor(value / -0x100000000);
			this.validateBuffer(ByteArraySize.SIZE_OF_UINT64);
			this.data.setUint32(this._position, low, this.$endian == EndianConst.LITTLE_ENDIAN);
			this.data.setInt32(this._position + 4, hi, this.$endian == EndianConst.LITTLE_ENDIAN);
			this.position += ByteArraySize.SIZE_OF_UINT64;
		}
	}
	...
	public writeUInt64(value: number): void {
		this.validateBuffer(ByteArraySize.SIZE_OF_UINT64);
		this.data.setUint32(this._position, value & 0xFFFFFFFF, this.$endian == EndianConst.LITTLE_ENDIAN);
		this.data.setUint32(this._position + 4, Math.floor(value / 0x100000000) & 0xFFFFFFFF, this.$endian == EndianConst.LITTLE_ENDIAN);
		this.position += ByteArraySize.SIZE_OF_UINT64;
	}
	...
	public validate(len: number): boolean {
		...
		if (bl > 0 && this._position + len <= bl) {
			...
			console.error("read out of range");
		}
	}
...
// 	ByteArray.prototype.decodeUTF8 = function (data: Uint8Array): string {
// 		let decoder = new TextDecoder();
// 		return decoder.decode(data);
// 	}
// }
// if (typeof TextEncoder !== 'undefined') {
// 	ByteArray.prototype.encodeUTF8 = function (str: string): Uint8Array {
